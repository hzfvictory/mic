<template>
  <div id="app">
    ...
    <div @click="onMore('值')">父组件</div>

    <hr />
    <strong class="tempCom">组件compA</strong>
    .lazy .number .trim select elementUI v-for @keyup.enter @[event]="doSomething" $nextTick $el
    <hr />

    <compA />

    <hr />
    <strong class="tempCom">组件compB</strong>
    .sync slot @ keep-alive ref v-once 自定义指令 管道符 过滤器 deactivated(){} activited(){}
    <hr />
    ...
    <hr />
    <strong class="tempCom">组件compC</strong>
    JSX $parent
    <hr />
    <CompC :name.sync="name" :onChange="onMore">
      找数据 11 1 1 1
      <h1 slot="namedSlot">222</h1>
    </CompC>

    <hr />
    <strong class="tempCom">Other</strong>
    :is radio transition
    <hr />
    ...
    <transition name="component-fade" mode="out-in">
      <keep-alive>
        <component v-bind:is="view">这里面也是可以添加数据的</component>
      </keep-alive>
    </transition>
  </div>
</template>
