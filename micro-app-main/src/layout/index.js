import React from "react"
import {Layout, Menu, Breadcrumb, Dropdown} from 'antd';
import {AppstoreOutlined, SettingOutlined, CaretDownOutlined, LogoutOutlined} from '@ant-design/icons';
import {renderRoutes} from 'react-router-config'
import {Link} from "react-router-dom";
import startQiankun from "../micro";

import "./index.css"

const menus = [
  {
    key: "Home",
    title: "去菜单二",
    path: "/menu2/static"
  },
  {
    key: "MicroAppList",
    title: "主应用",
    path: "/menu/list"
  },
  {
    key: "ReactMicroApp",
    title: "react微应用",
    path: "/menu/react"
  },
  {
    key: "StaticMicroApp",
    title: "vue微应用",
    path: "/menu/vue"
  },
  {
    key: "AngularMicroApp",
    title: "angular微应用",
    path: "/menu/angular"
  },
  {
    key: "StaticMicroApp",
    title: "Static微应用",
    path: "/menu/static"
  }
]


const {Header, Content, Footer} = Layout;

class Layouts extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      route: props.route.routes,
      current: props.location.pathname
    }
  }

  componentDidMount() {
    startQiankun();
  }

  // 跳转页面
  handleClick = e => {
    this.setState({
      current: e.key,
    });
    this.props.history.push(e.key)
  };

  // 获取当前路由信息
  curRoute = (type) => {
    const {route} = this.state;
    const {location: {pathname}} = this.props;
    const result = route.find((item) => item.path === pathname);
    return result[type]
  }

  onLogout = () => {
    console.log(1212);
  };

  findMunu() {
    const {current} = this.state;
    let curMenu = menus.find((item) => item.path === current);
    return curMenu || {}
  }

  render() {
    const {route, current} = this.state;
    const menu = (
      <Menu>
        <Menu.Item>
          <Link to="/set">
            <SettingOutlined style={{marginRight: 8}}/>
            个人设置
          </Link>
        </Menu.Item>
        <Menu.Item onClick={this.onLogout}>
          <LogoutOutlined style={{marginRight: 8}}/>
          退出登录
        </Menu.Item>
      </Menu>
    );

    return (
      <Layout>
        <Header style={{position: 'fixed', zIndex: 1, width: '100%'}}>
          <div className="logo"/>

          <span className={'userSet'}>
             <Dropdown overlay={menu}>
                  <span className="ant-dropdown-link">
                       <img alt="" className={'avatar'}
                            src={'data:image/png;base64,'}/>
                  禾口和 10000 <CaretDownOutlined/>
                  </span>
            </Dropdown>
          </span>


          {/*顶部菜单栏*/}
          <Menu theme="dark" onClick={this.handleClick} selectedKeys={[this.state.current]} mode="horizontal">
            {
              menus.map((item) => {
                return <Menu.Item key={item.path} icon={<AppstoreOutlined/>}>
                  {item.title}
                </Menu.Item>
              })
            }
          </Menu>
        </Header>

        {/*内容区域*/}
        <Content className="site-layout" style={{padding: '0 50px', marginTop: 64}}>
          <Breadcrumb style={{margin: '16px 0'}}>
            <Breadcrumb.Item href={"/menu/home"}>首页</Breadcrumb.Item>
            <Breadcrumb.Item>{this.findMunu().title}</Breadcrumb.Item>
          </Breadcrumb>
          {/*操作展示区域*/}

          <div id={'frame'} className="site-layout-background" style={{padding: 24, minHeight: 380}}/>
          {
            route.find(item => item.path === current) &&
            <div className="site-layout-background"
                 style={{padding: 24, minHeight: 380}}>
              {renderRoutes(route)}
            </div>
          }

        </Content>

        {/*底部*/
        }
        <Footer style={{textAlign: 'center'}}>xxxx-xxxx-{this.findMunu().title}</Footer>
      </Layout>
    );
  }
}

export default Layouts
